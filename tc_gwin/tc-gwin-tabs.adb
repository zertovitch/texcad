...
package body TC.GWin.Tabs is

  use GWindows.Menus;

  function Item_under_Mouse_Cursor (Control : in out TeXCAD_Tab_Bar_Type) return Integer is
    (Control.Item_At_Position
      (Control.Point_To_Client
        (GWindows.Cursors.Get_Cursor_Position)));

  procedure Refresh_Tool_Tip (Control : in out TeXCAD_Tab_Bar_Type) is
  ...
  begin
    ...
    then
      ...
      declare
        window_of_tab_under_pointer : MDI_Picture_Child.MDI_Picture_Child_Type
          renames
            MDI_Picture_Child.MDI_Picture_Child_Type
              (Control.info (tab_under_pointer).Window.all);
        fn : constant GWindows.GString :=
  ...
  overriding procedure On_Change (Control : in out TeXCAD_Tab_Bar_Type) is
  begin
  ...
  overriding procedure On_Create (Control : in out TeXCAD_Tab_Bar_Type) is
  ...
  overriding procedure On_Message
     (Window       : in out TeXCAD_Tab_Bar_Type;
  ...
  overriding procedure On_Middle_Click (Control : in out TeXCAD_Tab_Bar_Type) is
  ...
  overriding procedure On_Right_Click (Control : in out TeXCAD_Tab_Bar_Type) is
    chosen_tab : constant Integer := Item_under_Mouse_Cursor (Control);
    chosen_child_window : GWindows.Windows.MDI.Pointer_To_MDI_Child_Window_Class;
  ...
  overriding procedure Delete_Tab (Control : in out TeXCAD_Tab_Bar_Type; Where : in Integer) is
  begin
    if Where >= 0 then
  ...
  function Tab_Index (Control : in out TeXCAD_Tab_Bar_Type; ID : ID_Type) return Integer is
  begin
    for tab_idx in 0 .. Control.Tab_Count - 1 loop
      if Equivalent (ID, Control.info (tab_idx).ID) then
        return tab_idx;
      end if;
    end loop;
    return -1;
